...
const loadWorkouts = () => {
    try {
        ...
        entries.forEach((workout) => {
            ...
            $workout.querySelector('i')?.addEventListener('click', (e) => {
                e.stopPropagation();
...
const showWorkoutInModal = (workout) => {
    ...
    if (!isEditable)
        enableCompletedSection(); // Ensure completed section is editable in view mode
...
const enableCompletedSection = () => {
    ...
    if ($completedSection) {
        Array.from($completedSection.querySelectorAll('input')).forEach((element) => {
            if (element instanceof HTMLInputElement)
...
const validateWorkout = (workout) => workout.duration.hrs >= 0 &&
    workout.duration.mins >= 0 &&
    workout.duration.secs >= 0 &&
    workout.distance >= 0;
const isWorkoutCompleted = (workout) => workout?.completion
